"""
Pre-compute LamRA-Rank-Qwen2.5VL-7B teacher scores over a sampled LaSCo subset.

Offline distillation counterpart of scripts/precompute_teacher_scores.py, which
uses Qwen3-VL-2B.  Reuses the same 40K subset (data/lasco/distill_subset.json)
so results are directly comparable between teachers.

Outputs
-------
  data/lasco/distill_subset.json       — 40K sampled triplets (reused if exists)
  data/lasco/teacher_scores_lamra.json — per-triplet pos + K neg scores

Usage
-----
  python scripts/precompute_teacher_scores_lamra.py \\
      --config configs/distillation/precompute_lamra.yaml
...
def _build_reranker(cfg: dict) -> LamRARanker:
    rc = cfg["reranker"]
    img = rc.get("image_io", {})
    return LamRARanker(
...
def main(args: argparse.Namespace) -> None:
    ...
    logger.info(f"Teacher    : LamRA-Rank-Qwen2.5VL-7B")
    ...
    # Reuse existing subset so LamRA scores are on the same 40K triplets as Qwen.
    ...
    # Same seed as Qwen script so negative sets are identical.
    ...
    # ── Load reranker ────────────────────────────────────────────────────
    logger.info("Loading LamRA-Rank-Qwen2.5VL-7B reranker …")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pre-compute LamRA teacher scores on a LaSCo subset."
    )
    parser.add_argument(
        "--config", type=str,
        default="configs/distillation/precompute_lamra.yaml",
